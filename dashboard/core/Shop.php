...
<div class="container">
	...
	?>
		...
		<td><a href="<?php echo SITE;?>shop/edit/<?php echo $db->intcast($result[$i]['id']);?>/"><?php echo  $db->clean($result[$i]['product.title'],'encode');?></a></td>
		<td><?php echo $db->intcast($result[$i]['product.stock']);?> left in stock</td>
		<td><a href="<?php echo SITE;?>shop/edit/<?php echo $db->intcast($result[$i]['id']);?>/"><span class="material-symbols-outlined">edit</span></a></td>
		<td width="80"><a href="<?php echo SITE . 'shop/'.$token;?>/delete/<?php echo $db->intcast($result[$i]['id']);?>/" onclick="return confirm('Are you sure you want to remove this shop item?');"><span class="material-symbols-outlined">delete</span></a></td>
		</tr>
